...
class JackTokenizer():
    ...
    def _clearInineCmt(self, codeLine):
        temp = ''
        for i in range(len(codeLine)):
            if codeLine[i] == '/' and codeLine[i+1] == '/' :
                break
    ...
    def _tokenize(self, sourceCode):   # Pass the jack source code list and it returns Jack tokens
        multCmtMode = None
        stringMode = None
        temp = ""
        tempTokens = []
        ...
        for line in sourceCode: # Loop through each line in source code
            ...
            if self._isNewLine(line):
                ...
                currentLine = self._clearInineCmt(line).strip()

            # Removed unwanted spaces from the left
            # Handle the comments
            if self._isMultCmt(currentLine):  # Check if current line contains multi-line comment.

                if self._isMultCmtEnd(currentLine): # If this comment ended in one line, go to the next line.
                    continue
                else:
